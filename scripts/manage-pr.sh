...
: "${BASE_BRANCH:?BASE_BRANCH env var is required}"
COMMIT_PREFIX="${COMMIT_PREFIX:-}"
PR_LABELS="${PR_LABELS:-}"

if [[ -n "$COMMIT_PREFIX" ]]; then
  PR_TITLE="${COMMIT_PREFIX} update GitHub Actions to latest versions"
else
  PR_TITLE="Update GitHub Actions to latest versions"
fi

LABEL_ARG=()
if [[ -n "$PR_LABELS" ]]; then
  LABEL_ARG=(--label "$PR_LABELS")
fi
