...
while IFS='|' read -r action _ _ latest_sha latest_tag; do
  ...
  for workflow in .github/workflows/*.yml; do
    if grep -q "uses:[[:space:]]*${action}@" "$workflow"; then
      ...
      # Use `-i.bak` + rm for portability: GNU sed accepts `-i ""`, BSD sed
      # (macOS) requires an extension argument. `.bak` works on both.
      sed -i.bak "s|uses:\([[:space:]]*\)${action}@[^[:space:]#]*.*|uses:\1${action}@${latest_sha} # ${latest_tag}|" "$workflow"
      rm -f "${workflow}.bak"
      CHANGES_MADE=1
    fi
  done
